...
Namespace Handlers
    ...
    Public Class Blob
        ...
        Private Sub RenderUploader(ByVal context As HttpContext, ByVal handler As BlobHandlerInfo, ByVal uploadSuccess As Boolean)
            ...
            Dim script As String = "" & ControlChars.CrLf &"                       " & ControlChars.CrLf &"function ShowUploadControls() { " & ControlChars.CrLf &"    document.getElementById('UploadControlsPanel').style.display ='block'; " & ControlChars.CrLf &"    document.getElementById('StartUploadPanel').style.display = 'none';   " & ControlChars.CrLf &"    document.getElementById('FileUpload').focus();      " & ControlChars.CrLf &"} " & ControlChars.CrLf &"function Owner() {" & ControlChars.CrLf &"    var m = window.location.href.match(/owner=(.+?)&/);" & ControlChars.CrLf &"    return m ? parent.$find(m[1]) : null;" & ControlChars.CrLf &"}" & ControlChars.CrLf &"function StartUpload(msg) {" & ControlChars.CrLf &"    if (msg && !window.confirm(msg)) return;" & ControlChars.CrLf &"    if (parent && parent.window.Web) {" & ControlChars.CrLf &"        var m = window.location.href.match(/&index=(\d+)$/);" & ControlChars.CrLf &"        if (m) Owner()._showUploadProgress(m[1], document.forms[0]);" & ControlChars.CrLf &"    }" & ControlChars.CrLf &"}" & ControlChars.CrLf &"function UploadSuccess(key, message) { " & ControlChars.CrLf &"    if (!Owner().get_isInserting())" & ControlChars.CrLf &"        if (parent && parent.window.Web) { " & ControlChars.CrLf &"            parent.Web.DataView.showMessage(message); " & ControlChars.CrLf &"            Owner().refresh(false,null,'FIELD_NAME');" & ControlChars.CrLf &"        }     " & ControlChars.CrLf &"        else " & ControlChars.CrLf &"            alert('Success');" & ControlChars.CrLf &"}"
    ...
    Public Class BlobAdapter
        ...
        Public Shared ReadOnly Property ValidationKey() As String
            Get
                Return "FEF77B86F46EF3C472527A26C2B86DD881EEE6840C8C2A88343AA0D1C590E3198C7C93DA4758119AD2DB0352D660D8B7E17948CC3E25F7444F6C2CA85A01F458"
            End Get
        End Property
