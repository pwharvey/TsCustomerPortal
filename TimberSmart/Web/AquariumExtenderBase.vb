...
Namespace Web
    ...
    Public Class AquariumExtenderBase
        ...
        Protected Overrides Function GetScriptReferences() As System.Collections.Generic.IEnumerable(Of ScriptReference)
            ...
            Dim assemblyFullName As String = [GetType]().Assembly.FullName
            scripts.Add(New ScriptReference("TimberSmart._System.js", assemblyFullName))
            If isMobile Then
                scripts.Add(New ScriptReference(String.Format("~/touch/jquery.mobile-{0}.min.js", ApplicationServices.JqmVersion)))
            End If
            scripts.Add(New ScriptReference("TimberSmart.MicrosoftAjax.js", assemblyFullName))
            If Not (isMobile) Then
                scripts.Add(New ScriptReference("TimberSmart.AjaxControlToolkit.js", assemblyFullName))
                scripts.Add(New ScriptReference("TimberSmart.MicrosoftAjaxWebForms.js", assemblyFullName))
            End If
            scripts.Add(New ScriptReference(CultureManager.ResolveEmbeddedResourceName("TimberSmart.daf-resources.js"), assemblyFullName))
            If Not (isMobile) Then
                scripts.Add(New ScriptReference("TimberSmart.daf-menu.js", assemblyFullName))
            End If
            scripts.Add(New ScriptReference("TimberSmart.daf.js", assemblyFullName))
            If (Not (isMobile) AndAlso New ControllerUtilities().SupportsScrollingInDataSheet) Then
                scripts.Add(New ScriptReference("TimberSmart.daf-extensions.js", assemblyFullName))
            End If
            If EnableCombinedScript Then
                scripts.Add(New ScriptReference("TimberSmart.daf-membership.js", assemblyFullName))
            End If
            ConfigureScripts(scripts)
            If isMobile Then
                scripts.Add(New ScriptReference("TimberSmart.touch.js", assemblyFullName))
                scripts.Add(New ScriptReference("TimberSmart.touch-charts.js", assemblyFullName))
            End If
        ...
        Protected Overrides Sub OnPreRender(ByVal e As EventArgs)
            ...
            For Each c As Control in Page.Header.Controls
                If (c.ID = "TimberSmartTheme") Then
                    Return
                End If
            Next
            Dim link As HtmlLink = New HtmlLink()
            link.ID = "TimberSmartTheme"
            link.Href = Page.ClientScript.GetWebResourceUrl(GetType(AquariumExtenderBase), "TimberSmart.Aquarium.css")
